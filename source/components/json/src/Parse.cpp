...
{
    return OSAL::Strings::isalnum(ch) || (ch == '-') || (ch == '+') || (ch == '.');
}
...
Token GetToken(std::istream & stream)
{
    ...
    if (OSAL::Strings::IsEqualIgnoreCase(term, "null"))
        return Token { TokenType::NullToken, term };
    if (OSAL::Strings::IsEqualIgnoreCase(term, "false"))
        return Token { TokenType::FalseToken, term };
    if (OSAL::Strings::IsEqualIgnoreCase(term, "true"))
        return Token { TokenType::TrueToken, term };
    if (OSAL::Strings::IsEqualIgnoreCase(term, "{"))
    ...
    if ((term.length() >= 1) && (OSAL::Strings::isdigit(term[0]) || (term[0] == '-')))
    {
        size_t index = 0;
        if (term[index] == '-')
        {
        ...
        while ((index < term.length()) && (OSAL::Strings::isdigit(term[index])))
            ++index;
        if ((index < term.length()) && OSAL::Strings::tolower(term[index]) == '.')
        {
            ++index;
            while ((index < term.length()) && (OSAL::Strings::isdigit(term[index])))
                ++index;
        }
        if ((index < term.length()) && OSAL::Strings::tolower(term[index]) == 'e')
        {
            ++index;
