...
{
    std::basic_istringstream<char> stream("false true null garbage");
...
TEST_FIXTURE(ParseTest, ParseString)
{
    ...
    EXPECT_EQ("false true null garbage", static_pointer_cast<String>(target)->GetValue());

    target = Parse(stream);
    EXPECT_NOT_NULL(target);
    EXPECT_EQ(ValueType::String, target->Type());
    EXPECT_EQ("", static_pointer_cast<String>(target)->GetValue());
...
{
    std::basic_istringstream<char> stream("0123 123.456 1.001e+03 abc");
    ValuePtr target = Parse(stream);
    EXPECT_NOT_NULL(target);
    EXPECT_EQ(ValueType::Number, target->Type());
    EXPECT_EQ("0123", static_pointer_cast<Number>(target)->GetValue());

    target = Parse(stream);
    EXPECT_NOT_NULL(target);
    EXPECT_EQ(ValueType::Number, target->Type());
    EXPECT_EQ("123.456", static_pointer_cast<Number>(target)->GetValue());

    target = Parse(stream);
    EXPECT_NOT_NULL(target);
    EXPECT_EQ(ValueType::Number, target->Type());
    EXPECT_EQ("1.001e+03", static_pointer_cast<Number>(target)->GetValue());
...
{
    std::basic_istringstream<char> stream("{ \"x\":true, \"y\":1234 }");
    ValuePtr target = Parse(stream);
...
{
    std::basic_istringstream<char> stream("[ true, 1234 ]");
    ValuePtr target = Parse(stream);
    ASSERT_NOT_NULL(target);
