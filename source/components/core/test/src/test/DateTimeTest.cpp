...
TEST_FIXTURE(DateTimeTest, TimeZoneNameLocal)
{
    ...
    time_t time = dateTime;
    OSAL::Time::tm localTime = *OSAL::Time::localtime(&time);
    std::string nameExpected = localTime.tm_tzName;
    std::string nameActual = dateTime.TimeZoneName();
...
#else
    // In Windows, %F and %T do not work correctly, so they are replaced with %Y-%m-%d and %H:%M:%S
    dateTime.PrintTo(stream, "%Y-%m-%d %H:%M:%S");
#endif
...
#else
    // In Windows, %F and %T do not work correctly, so they are replaced with %Y-%m-%d and %H:%M:%S
    dateTime.PrintTo(stream, "%Y-%m-%d %H:%M:%S %Z");
    // This should print as UTC, but doesn't on Windows
    EXPECT_EQ("2014-02-26 01:02:03 W. Europe Standard Time", stream.str());
#endif
}
