...
bool CommandLineParser::Parse(int argc, const char * argv[])
{
    ...
    {
        _console << fgcolor(OSAL::ConsoleColor::Red) << "Exception: " << e.what() << endl;
        if (_autoHelp)
        {
            ShowHelp(argv[0]);
        }
...
bool CommandLineParser::InternalParse(int argc, const char * argv[])
{
    ...
    while (result != -1)
    {
        ...
        if (result == 0)
        {
            ...
            auto optionWithVariable
                = dynamic_pointer_cast<CommandLineSwitchWithVariable<int>>(option);
                                        ...
                                        bool printErrors)
{
    ...
    for (auto currentOption : _options)
    {
        ...
        {
            auto currentOptionWithVariable
                = dynamic_pointer_cast<CommandLineSwitchWithVariable<int>>(currentOption);
            auto optionFoundWithVariable
                = dynamic_pointer_cast<CommandLineSwitchWithVariable<int>>(optionFound);
    ...
    if (!ambiguousOptionsList.empty() && !exact)
    {
        if (printErrors)
        {
            ...
            _console << fgcolor(OSAL::ConsoleColor::Red) << argv[0] << ": option '"
                     << argv[_getOptData.optionIndex] << "' is ambiguous; possibilities:";
        ...
        if (*nameEnd)
        {
            ...
            else
            {
                if (printErrors)
                {
                    ...
                    {
                        _console << argv[0] << ": option '--" << optionFound->LongName()
                                 << "' doesn't allow an argument";
                    }
                    else
                    {
                        _console << argv[0] << ": option '--" << argv[_getOptData.optionIndex - 1][0]
                                 << optionFound->LongName() << "' doesn't allow an argument";
                    }
                    _console << fgcolor(OSAL::ConsoleColor::Default) << endl;
                }
                _getOptData.nextChar += OSAL::Strings::strlen(_getOptData.nextChar);
    ...
    if (printErrors)
    {
        ...
        {
            _console << argv[0] << ": unrecognized option '--" << argv[_getOptData.optionIndex][0]
                                         ...
                                         bool printErrors)
{
    ...
    const char * foundShortOption = OSAL::Strings::strchr(optionString, c);
                              ...
                              size_t & optionIndex)
{
    ...
    if ((nullptr == _getOptData.nextChar) || ('\0' == *_getOptData.nextChar))
    {
        ...
        if ((_getOptData.optionIndex < argCount)
            && !OSAL::Strings::strcmp(argv[_getOptData.optionIndex], "--"))
        {
...
{
    _console << fgcolor(OSAL::ConsoleColor::Yellow) << GetHelp(applicationName) << endl;
...
string CommandLineParser::GetHelp(const string & applicationName,
