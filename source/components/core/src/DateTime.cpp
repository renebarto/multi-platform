...
static int ConvertMonth(MonthType monthType)
{
    return int(monthType);
}

static MonthType ConvertMonth(int month)
{
    return MonthType(month);
}

static int ConvertWeekDay(WeekDayType weekDay)
{
    return int(weekDay);
}

static WeekDayType ConvertWeekDay(int weekDay)
{
...
{
    OSAL::Time::tm local(static_cast<int>(second), minute, hour, day, month - 1, year - 1900);
    ...
    _time.tv_nsec = static_cast<long>((second - static_cast<int>(second)) * NanoSecondsPerSecond + 0.5);
}
