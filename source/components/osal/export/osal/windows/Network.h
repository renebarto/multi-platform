    ...
    void * optionValue, unsigned int optionLength)
{
    return ::setsockopt(socketHandle,
        ...
        reinterpret_cast<char *>(optionValue), static_cast<int>(optionLength));
}
...
{
    return ::recv(socketHandle, reinterpret_cast<char *>(data), static_cast<int>(bufferSize), flags);
}

inline OSAL_EXPORT ssize_t Send(SocketHandle socketHandle, const uint8_t * data, size_t bufferSize, int flags)
{
    return ::send(socketHandle, reinterpret_cast<const char *>(data), static_cast<int>(bufferSize), flags);
}
