...
CommandLineOptionsParser::CommandLineOptionsParser(OSAL::Console & console)
    ...
    , xmlOutput()
{
    AddOptionRequiredArgument("runsuite", 's',
                              "Run only specified test suite (default = all)",
                              &testSuiteName);
    AddOptionRequiredArgument("runfixture", 'f',
                              "Run only specified test fixture (default = all)",
                              &testFixtureName);
    AddOptionRequiredArgument("runtest", 't',
                              "Run only specified test (default = all)", &testName);
    AddOptionRequiredArgument("xmloutput", 0, "Send output to specified XML file",
                              &xmlOutput);
}
