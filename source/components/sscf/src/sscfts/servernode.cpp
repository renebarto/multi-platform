...
{
    return invoke<bool, const std::string>(hostId, "sscf", "/providesType", name);
}

std::string ServerNode::createInstance(const std::string &clientHostId, const char *name)
{
    const std::string result = invoke<std::string, const std::string, const std::string>(clientHostId, "sscf", "/createInstance", name, empty);
	if (result.empty())
...
{
    const std::string result = invoke<std::string, const std::string, const std::string>(clientHostId, "sscf", "/createInstance", name, instance);
	if (result.empty())
		throw CommunicationException("Failed to create proxy for: " + std::string(name) + " at [" + clientHostId + ']');
...
Sscf1::BulkChannel & ServerNode::bulkChannel()
{
...
void ServerNode::sendNotification(const std::vector<std::string> &hostIds, const std::vector<Sscf1::BulkChannel::BulkId> &bulkIds, const std::string &instance, const char *path, Message &in)
{
    std::ostringstream fullPath;
	fullPath.imbue(std::locale::classic());
	fullPath << "/" << instance << path;
...
void ServerNode::sendDestroyInstance(const std::string &clientHostId, const std::string &instance)
{
    if (p->server->isConnected(clientHostId.c_str()))
    {
        try
        {
            invoke<void, const std::string>(clientHostId, "sscf", "/destroyInstance", instance);
        }
