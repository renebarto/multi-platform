  ...
  : p(new Private())
{
    ...
    registerProxy(nullptr, "sscf", "/providesType", &Dispatcher::providesType, this);
    registerProxy(nullptr, "sscf", "/createInstance", &Dispatcher::createInstance, this);
    registerProxy(nullptr, "sscf", "/destroyInstance", &Dispatcher::destroyInstance, this);
}
...
EventBase::Subscription Dispatcher::subscribeNotificationDelegate(Event<void()> &event, const std::string &instance, const char *name, const char *path)
{
    return event.subscribe([this, instance, name, path]
    {
        ...
        sendNotification(
            ...
            std::vector<Sscf1::BulkChannel::BulkId>(),
            instance,
            path,
            in);
    });
