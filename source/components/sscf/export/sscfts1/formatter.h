...
#ifndef SSCFTS1_FORMATTER_H
#define SSCFTS1_FORMATTER_H
...
/*! Class to serialize objects to a buffer. To make custom serializable objects
    ...
    SscfTs1::Formatter::Serializer & operator<<(SscfTs1::Formatter::Serializer &, const MyClass &);
    Make sure binary serialization is done as newtork byteorder, the
    SscfTs1::Formatter::eswap() function will swap the bytes if needed.
 */
class SSCF_EXPORT Serializer final
{
public:
    Serializer(std::vector<uint8_t> &buffer, Sscf1::BulkChannel * = nullptr);
    ~Serializer();

    Serializer & operator<<(const BulkData &val);

    const std::vector<Sscf1::BulkChannel::BulkId> & bulkIds() const { return serializedBulkIds; }
    ...
    /*! The operator<< can be used to serialize data, for example:
        \code
            ...
            {
                SscfTs1::Formatter::Serializer serializer(buffer);
            ...
            {
                SscfTs1::Formatter::Deserializer deserializer(buffer);
...
private:
    std::vector<uint8_t> &buffer;
    Sscf1::BulkChannel * const bulkChannel;
    std::vector<Sscf1::BulkChannel::BulkId> serializedBulkIds;
#endif // __INTELLISENSE__
};
...
/*! Class to deserialize objects from a buffer. To make custom deserializable objects
    define an operator>> for this type in the namespace of the type:
    SscfTs1::Formatter::Deserializer & operator>>(SscfTs1::Formatter::Deserializer &, MyClass &);
    Make sure binary serialization is done as newtork byteorder, the
    SscfTs1::Formatter::eswap() function will swap the bytes if needed.
 */
class SSCF_EXPORT Deserializer final
{
public:
    Deserializer(const std::vector<uint8_t> &buffer, Sscf1::BulkChannel * = nullptr);
    ~Deserializer();
