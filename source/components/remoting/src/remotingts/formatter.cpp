...
///////////////////////////////////////////////////////////////////////////////
// Serializer
Serializer::Serializer(std::vector<uint8_t> &buffer, Remoting1::BulkChannel *bulkChannel)
  : buffer(buffer),
...
Serializer & Serializer::operator<<(const BulkData &val)
{
	if (bulkChannel)
	{
		const Remoting1::BulkChannel::BulkId bulkId = bulkChannel->enqueue(val);
		serializedBulkIds.push_back(bulkId);

		return write(bulkId.data, sizeof(bulkId.data));
	}
	else
		Remoting1::system().fail("Serializing BulkData while BulkChannel not available.");
}
...
///////////////////////////////////////////////////////////////////////////////
// Deserializer
Deserializer::Deserializer(const std::vector<uint8_t> &buffer, Remoting1::BulkChannel *bulkChannel)
  : buffer(buffer), 
	readPos(0),
	bulkChannel(bulkChannel)
{
...
Deserializer & Deserializer::operator>>(BulkData &val)
{
	if (bulkChannel)
	{
		Remoting1::BulkChannel::BulkId bulkId;
