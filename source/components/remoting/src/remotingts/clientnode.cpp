...
{
	if (Remoting1::system().threadSafetyChecksEnabled() &&
		!p->eventLoop->checkThread())
	{
		Remoting1::system().fail(
...
{
    return invoke<bool, const std::string>(empty, "remoting", "/providesType", name);
}

std::string ClientNode::createInstance(const char *name)
{
    const std::string result = invoke<std::string, const std::string, const std::string>(empty, "remoting", "/createInstance", name, std::string());
	if (result.empty())
...
{
    const std::string result = invoke<std::string, const std::string, const std::string>(empty, "remoting", "/createInstance", name, instance);
	if (result.empty())
		throw CommunicationException("Failed to create proxy for: " + std::string(name));
...
Remoting1::BulkChannel & ClientNode::bulkChannel()
{
...
void ClientNode::sendNotification(const std::vector<std::string> &, const std::vector<Remoting1::BulkChannel::BulkId> &, const std::string &instance, const char *path, Message &in)
{
    std::ostringstream fullPath;
	fullPath.imbue(std::locale::classic());
...
void ClientNode::sendDestroyInstance(const std::string &, const std::string &instance)
{
    if (p->client->isConnected())
    {
        try
        {
            invoke<void, const std::string>(empty, "remoting", "/destroyInstance", instance);
        }
