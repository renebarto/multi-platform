...
public:
    ...
    {
        std::cout << "BEG SSCFServer::StartServer" << std::endl;
        ipAddress_ = ipAddress;
        
        eventLoop_ = std::make_unique<SscfTs1::EventLoop>(ITestUrn);
        implementation_ = std::make_unique<Implementation>(eventLoop_.get());
        eventLoop_->send([this]
        {
            serverNode_ = std::make_unique<SscfTs1::ServerNode>(*eventLoop_, ipAddress_, ITestUrn);
        ...
        std::cout << "END SSCFServer::StartServer" << std::endl;
    }
    void RunServerSync()
    {
        std::cout << "BEG SSCFServer::RunServerSync" << std::endl;
        eventLoop_->run();
        std::cout << "END SSCFServer::RunServerSync" << std::endl;
    }
    void RunServerAsync()
    {
        std::cout << "BEG SSCFServer::RunServerAsync" << std::endl;
        eventLoop_->start();
        std::cout << "END SSCFServer::RunServerAsync" << std::endl;
    }
    ...
    void StopServer()
    {
        if (eventLoop_ == nullptr)
            return;
        std::cout << "BEG SSCFServer::StopServer" << std::endl;
        eventLoop_->send([this]
        {
            if (serverNode_)
            {
        ...
        std::cout << "END SSCFServer::StopServer" << std::endl;
    }

private:
    std::unique_ptr<Implementation> implementation_;
    std::string ipAddress_;
    std::unique_ptr<SscfTs1::ServerNode> serverNode_;
    std::unique_ptr<SscfTs1::EventLoop> eventLoop_;
};
